...
public class TulingReadWriteLock implements ReadWriteLock {
    ...
    private void testReadLock(ReadLock readLock){
        new Thread(()->{
            readLock.lock();
            try {
    ...
    private void testWriteLock(WriteLock writeLock){
        new Thread(()->{
            writeLock.lock();
            try {
                System.out.println(Thread.currentThread().getName()+":获取了写锁==");
        ...
        TulingReadWriteLock tulingReadWriteLock = new TulingReadWriteLock();
        ReadLock readLock = tulingReadWriteLock.readLock;
        WriteLock writeLock = tulingReadWriteLock.writeLock;

        /**
         * 测试读锁：读写锁不会阻塞其他线程获取读锁
         */
        //tulingReadWriteLock.testReadLock(readLock);


        /**
         * 测试写锁：读写锁会阻塞写。  一定要等线程1释放了写锁后线程2才能获取到写锁
         */
        tulingReadWriteLock.testWriteLock(writeLock);
    }
